...
### GET
#### 1. GET 메서드는 주로 데이터를 읽거나(Read) 검색(Retrieve)할 때 사용되는 메서드다.
#### 2. HTTP 명세에 의하면 GET 요청은 오로지 데이터를 읽을 때만 사용되고 수정할 때는 사용하지 않는다.
#### 3. GET 요청은 멱등성(idempotent) 하다. 여기서 말하는 멱등성(idempotent)란 연산을 여러번 적용하더라도 결과가 달라지지 않는 성질을 의미한다.
#### 4. 데이터를 변경하는 연산에 사용하면 안된다.
### POST
#### 1. POST 메서드는 주로 새로운 resource를 생성할 때 사용된다.
#### 2. POST 요청은 멱등성(idempotent) 하지 않다.
#### 3. 같은 POST 요청을 반복해서 했을 때 항상 같은 결과물이 나오는 것을 보장하지 않는다.
#### 4. 두개의 같은 POST 요청을 보내면 같은 정보를 담은 두개의 다른 resource를 반환할 가능성이 높다.
### PUT
#### 1. PUT 메서드는 resource를 생성 및 업데이트하기 위해 server로 데이터를 보내는데 사용된다.
#### 2. PUT 요청은 멱등성(idempotent) 하다.
